...
function showData($data,$connexion,$limit,$adjacent){
  ...
  $str='<table><tr class="head"><td>ID</td><td>num carte</td><td>Nom</td><td>Prenom</td><td>Email</td><td>Pass</td><td>Specialité</td><td>status</td><td>Action</td></tr>';   
  if($data->num_rows>0){
   while( $row = $data->fetch_array(MYSQLI_ASSOC)){
   if($row['status']==1){

$str.= "<tr ><td>$row[user_id]</td><td>$row[num_carte]</td><td>$row[nom]</td><td>$row[prenom]</td><td>$row[email]</td><td>$row[pass]</td><td>$row[specialite]</td><td c='text-align:left'><span class='label label-info label-mini'>Due</span></td><td><form id='1'action='lecteur.php'  methode='GET'></form>
...
$str.= "<tr ><td>$row[user_id]</td><td>$row[num_carte]</td><td>$row[nom]</td><td>$row[prenom]</td><td>$row[email]</td><td>$row[pass]</td><td>$row[specialite]</td><td style='text-align:center'><span class='label label-danger label-mini'>banned</span></td><td><form id='1'action='lecteur.php'  methode='GET'></form>
       <form id='2' action='prets_ajout.php'  methode='GET'></form> 
	<button form='1' type='submit' name='mod' value='$row[user_id]'     class='btn btn-primary btn-xs'><i class='fa fa-pencil'></i></button>
		<button form='1' type='submit' name='supp' value='$row[user_id]'     class='btn btn-danger btn-xs'><i class='fa fa-trash-o '></i></button>
		<button form='2' type='submit' name='id_lec' value='$row[user_id]'     class='btn btn-primary btn-xs'><i class='fa fa-plus-square'></i>
